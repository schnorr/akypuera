    ...
    const *key, int size, int mark)
{
  ...
  poti_user_StartLink (specialStartLinkSizeMark, timestamp, "root", "LINK", mpi_process, keys, key, 2, size_, mark_);
}
  ...
  while (rst_decode_event(&rastro, &event) && !fail) {
    ...
    case MPI_FINALIZE_IN:
      if (!arguments.no_states){
        ...
        char extra[nb_extra][21];  /* uint64 upper range is a 20 digits integer in base 10 */
        int i_extra, i_array;
        ...
        if(nb_extra) {
          poti_user_PushState (specialPushState, timestamp, mpi_process, "STATE", value, nb_extra, extra);
        }else{
